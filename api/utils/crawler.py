...
class CrawlerKakaoMap:
    ...
    def make_file(self, fileName : str):
        try:
            filename = './utils/res/'+fileName+'.csv'
    ...
    def crawlMap(self, keyword : str):
        fileName = './utils/res/'+keyword+'.csv'
    ...
    def findAll(self, filename : Optional[str]):
        fileName = f'./utils/res/{filename}.csv'
    ...
    def findOpen(self, filename : Optional[str]):
        ...
        fileName = f'./utils/res/{filename}.csv'
        df = pd.read_csv(fileName, encoding='utf-8',sep=",")
        current = utils.get_time(utils)
        for idx,data in df.iterrows():
            time_element = data[2]
            # print(type(time_element))
            if type(time_element) == str:
                closing_time = time_element[-5:]
                if len(time_element)>4 and ":" in time_element:
                    # closing_time = time_element[-5:]
                    ########################################
                    # TODO : 여기에 시간 비교 함수 적용 
                    ########################################
                    print(closing_time)
